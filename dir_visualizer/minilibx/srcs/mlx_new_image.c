...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/26 20:33:16 by wescande          #+#    #+#             */
/*   Updated: 2016/12/31 15:39:40 by wescande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*mlx_int_new_xshm_image(t_xvar *xvar, int width, int height, int format)
{
	...
	img->format = format;
	img->shm.shmid =
		shmget(IPC_PRIVATE, (width + 32) * height * 4, IPC_CREAT | 0777);
	if (img->shm.shmid == -1)
	{
...
void	*mlx_int_new_image(t_xvar *xvar, int width, int height, int format)
{
	...
	img->width = width;
	img->height = height;
	img->pix = XCreatePixmap(xvar->display,
		xvar->root, width, height, xvar->depth);
	img->format = format;

...
/*   Updated: 2017/01/03 15:21:20 by wescande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int				check_add_room(t_lem *lem, char *line, int next)
{
	...
	{
		ft_dprintf(2, "lem_in: {red}Multiple room detected. (%s)\n{eoc}",
						split[0]);
		ret = 1;
	}
	if (!ret)
	{
		if (!(n_room = (t_room *)malloc(sizeof(t_room))))
			malloc_error("room");
		*n_room = (t_room) {ft_strdup(split[0]), next == 1 ? lem->ants : 0,
			next, 0, NULL};
...
int						set_room(t_lem *lem, char **line)
{
	...
	{
		ft_dprintf(2, "lem_in: {red}Error on room input.\n{eoc}");
		return (1);
	}
	return (0);
}
